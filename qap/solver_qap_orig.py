...
#             product[f_m[f_order[i]] * d_m[d_order[i]]].append((f_order[i], d_order[i]))
        return products, pairs
    
    def _create_cluster(n, d, f, sta, n_clusters):
...
#         product_term = _calculate_product(n, f, d)
        # board, reversed_board = _create_board(n)
        cost = 0
    ...
    def _sub(filePath):
        ...
        print(f'\n{fileName}')
        n_clusters = n // 30 # 25
#         clusters = _create_cluster(n, d, f, parameters["sta"], parameters["n_clusters"])
        ...
        for idx, cluster in enumerate(clusters):
            print('Size of cluster {}: {}'.format(idx, len(cluster)))
            if len(cluster) > 1:
