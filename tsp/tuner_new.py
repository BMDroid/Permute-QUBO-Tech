...
def pso_new(fileName, inst, rules, bounds, parameters, solve):
    # bounds  = {'min': [0.5, 0.5], 'max': [1, 1]}
    # settings = {'iteration_num': [10**7, 10**8]} =>

    # bounds = {'0A': (0.5 * max_edge, max_edge), '1B': (0.5 * max_edge, max_edge),
    #           '2iteration_num': [10**7, 10**8]}

    bounds_copy = bounds.copy()
    num_penalties = 0
    for k, v in sorted(bounds_copy.items()):
        if len(k) == 2 and k[-1].isalpha():
            bounds_copy[k] = (v[0] * getattr(inst, parameters['base']),
    ...
    save_epoch(fileName, -1, cur_cost, cur_cost, cur_mixed, min_pos,
               epoch_elapsed, parameters, bounds_copy)
    print("\nInitialization finished")
    print(f"Best solution found after initialization: {cur_cost}")
    print(f"Best mixed solution found after initialization: {cur_mixed}")
    ...
    for e in range(parameters["epoch"]):
        ...
        while not que.empty():
            ...
            epoch_min = min(sol, epoch_min)
            save_solution(fileName, e, pos, min_perm, min_cost, val,
                          elapsed_time, parameters, bounds_copy)
            if val < val_arr[idx]:
                best_pos_arr[idx] = pos
                val_arr[idx] = val
                ratio = max(cur_mixed / val_arr[idx], val_arr[idx] / cur_mixed)
        ...
        # save info about every epoch
        save_epoch(fileName, e, cur_cost, epoch_min, cur_mixed, min_pos,
                   epoch_elapsed, parameters, bounds_copy)
        total_elapsed += epoch_elapsed
        # print out the current best
        print("{}".format(Path(fileName).stem))
...
def pso_all(fileName, inst, rules, bounds, parameters, solve):
    # bounds  = {'min': [0.5, 0.5], 'max': [1, 1]}
    # settings = {'iteration_num': [10**7, 10**8]} =>
...
def abc(fileName, inst, rules, bounds, parameters, solve):
        ...
        return np.concatenate(
            [
                np.random.uniform(
                    ...
                    v[1],
                    size,
                ).reshape(size, 1) for k, v in sorted(bounds.items())
            ],
            axis=1,
        ).astype(float)

    def _fitness_thr(solve, pos, epoch, tag):
        nonlocal fileName
        nonlocal inst
        nonlocal rules
        nonlocal parameters
        ...
        while not que.empty():
            ...
            save_solution(fileName, str(epoch) + '_' + tag, p, min_perm, min_cost,
                          obj, elapsed_time, parameters, bounds_copy)
    ...
    for epoch in range(parameters['cycle']):
        ...
        employed_fitness = _fitness_thr(solve, employed_pos, epoch, 'employed')
        # update neighbors' positions for employed
        neighbor_pos = _update(employed_pos, food_pos)
        neighbor_fitness = _fitness_thr(solve, neighbor_pos, epoch,
                                        'employed_neighbor')
        mask = np.array(employed_fitness - neighbor_fitness > 0).reshape(
            employ, 1)
        # abaddon the employed if no improvement has been made for $limit$ epochs
        abandonment += mask
        if epoch % parameters['limit'] == 0:
            scout = np.array(abandonment == parameters['limit']).reshape(
                employ, 1)
            if np.sum(scout) >= 1:
                abandonment = np.zeros((employ, 1))
        employed_pos = employed_pos * mask + neighbor_pos * (1 - mask)
        employed_fitness = employed_fitness * mask + neighbor_fitness * (1 -
                                                                         mask)
        ...
        onseeker_fitness = _fitness_thr(solve, onseeker_pos, epoch, 'onseeker')
        neighbor_fitness = _fitness_thr(solve, neighbor_pos, epoch,
                                        'onseeker_neighbor')
        mask = np.array(onseeker_fitness - neighbor_fitness > 0).reshape(
            onseeker, 1)
        onseeker_pos = onseeker_pos * mask + neighbor_pos * (1 - mask)
        onseeker_fitness = onseeker_fitness * mask + neighbor_fitness * (1 -
                                                                         mask)
        epoch_elapsed = time.time() - epoch_start
        save_epoch(fileName, epoch, cur_cost, epoch_min, cur_mixed, min_pos,
                   epoch_elapsed, parameters, bounds_copy)
    total_elapsed += epoch_elapsed
    return cur_perm, cur_cost, cur_mixed, min_pos, best_epoch, total_elapsed
