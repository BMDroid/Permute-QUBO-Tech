...
def main_fsp(dirName, tuner, bounds, parameters, solve):
    def _sub(filePath):
        ...
        two_opt_mksp, _ = seq.two_opt(
            cur_perm,
            cur_mksp)  # conduct two opt given the best solution found
...
def solve_tsp(tsp, rules, A, num_run, num_iteration):
    ...
    if len(min_path) == 0:
        return A, [], float("inf"), float("inf"), elapsed_time
    return np.array([*A, num_iteration
                     ]), min_path, min_distance, min_distance, elapsed_time
...
def solve_tsp_avg(tsp, rules, A, num_run, num_iteration):
    ...
    # elapsed_time
    elapsed_time = time.time() - start + res["anneal_time"] / 1000
    return np.array([*A, num_iteration]), min_path, min_distance, np.mean(
        dis_list) if len(dis_list) > 0 else float("inf"), elapsed_time


def solve_tsp_avg_all(tsp, rules, A, num_run, num_iteration):
    ...
    elapsed_time = time.time() - start + res["anneal_time"] / 1000
    return np.array(
        [*A, num_run, num_iteration]), min_path, min_distance, np.mean(
            dis_list) if len(dis_list) > 0 else float("inf"), elapsed_time
...
def main_tsp(dirName, tuner, bounds, parameters, solve):
    def _sub(filePath):
        ...
        with open(infoName, "a") as f:
            f.write(f"{Path(fileName).stem},{best_epoch + 1}," +
...
def save_solution(fileName, e, pos, min_perm, min_cost, mixed_cost,
    ...
    with open(outputName, "a") as f:
        f.write(f"{e + 1 if isinstance(e, int) else e}," + ','.join([f"{pos[i]}"
...
def save_epoch(fileName, e, cur_cost, epoch_min, cur_mixed, min_pos,
    ...
    if not os.path.exists(outputName):
        with open(outputName, "a") as f:
            f.write(
                "Epoch, " + ', '.join(
                    [f"Current_Best_{k[1:]}" for k in sorted(bounds.keys())]) +
                ", Current_Best_Cost, Current_Best_Mixed, Current_Epoch_cost, Elapsed_Time\n"
            )
    with open(outputName, "a") as f:
        f.write(f"{e + 1 if isinstance(e, int) else e}," +
...
def get_results_path_new(variant, parameters):
    comb = f"{'Fixed' if parameters['fixed'] else 'Non-Fixed'}-M{parameters['mixed']}-R{parameters['num_run']}"
    if "pso" in variant:
        comb += f"-{'U' if parameters['U'] else ''}{parameters['topo']}-{'Random' if parameters['ini'] == 0 else 'Even'}-E{parameters['epoch']}-P{parameters['particles']}-W{parameters['w']}-C1{parameters['c1']}-C2{parameters['c2']}"
    else:
        comb += f"-E{parameters['max_evals']}"
