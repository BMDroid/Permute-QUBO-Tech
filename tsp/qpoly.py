...
LastEditTime: 2021-01-15 02:31:24
'''
...
class QPoly(object):
    ...
    def __init__(self, n=1024):
        ...
        )  # note input must be int, i have change it to float
        self.constant = 0
        self._size = n
    ...
    def power_(self, p=2):
        ...
        self.array = np.triu(a, k=1) + np.triu(
            a.T) + (2 * self.constant * np.diag(diag))
    ...
    def multiply_quadratic_binary_polynomial(self, poly):
        ...
        if np.count_nonzero(self.array -
                            np.diag(diag0)) > 0 or np.count_nonzero(
                                poly.array - np.diag(diag1)) > 0:
            raise RuntimeError('not quadratic')
        a = np.outer(diag0, diag1)
        self.array = np.triu(a, k=1) + np.triu(a.T) + (
            self.constant * np.diag(diag1)) + (poly.constant * np.diag(diag0))
